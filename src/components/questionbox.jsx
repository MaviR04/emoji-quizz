...
function QuestionBox(){
    ...
    const[HintShown,setHint] = useState({opacity:0})
    ...
    return(
        <div>
            ...
            <div className="text-4xl text-wrap sm:text-5xl sm:w-2xl w-full text-center bg-gray-50 p-9 font-bold text-black font-Inter">
                <div style={AnswerShown}>{movies[index].name}</div>
            </div>
            <div style={HintShown}  className=" text-5xl  sm:text-3xl sm:w-2xl w-full text-center bg-transparent rounded-b-xl p-9">
                {movies[index].hint}
            </div>
            ...
            <div className=' flex justify-center sm:gap-20 gap-10 p-2 font-poppins mt-4 sm:flex-row flex-col '>
                ...
                <button type="button" onClick={UpdateHint} className='bg-white transition-all duration-500 text-black p-2 rounded-2xl font-semibold hover:bg-black hover:text-white'>Show hint</button>
            </div>
        </div>
    )
}
